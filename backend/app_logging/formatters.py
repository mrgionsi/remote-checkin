...
class RequestFormatter(logging.Formatter):
    ...
    Formats HTTP-specific information in a structured way.
    """
    
    def format(self, record: logging.LogRecord) -> str:
        """
        Format HTTP request/response log record.
        
        Args:
            record: Log record with HTTP information
        ...
        if hasattr(record, 'request_data'):
            ...
            message = (
                f"{request_data.get('method', 'UNKNOWN')} "
                f"{request_data.get('path', '/')} "
                f"- Status: {request_data.get('status_code', 'N/A')} "
                f"- Duration: {request_data.get('duration', 'N/A')}ms "
                f"- IP: {request_data.get('remote_addr', 'N/A')}"
            )
            
            # Add user info if available
            if request_data.get('user_id'):
                message += f" - User: {request_data['user_id']}"
